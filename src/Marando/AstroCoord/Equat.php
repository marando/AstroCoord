...
class Equat
{
     ...
     *
     * @param Pressure|null    $pressure Atmospheric pressure
     * @param Temperature|null $temp     Atmospheric temperature
     * @param null             $humidity Relative humidity
     *
     * @return $this|Equat|Horiz
     * @throws Exception
     */
    public function apparent(
      ...
      $humidity = null
        ...
        // Check if already converted to apparent, and return that if so
        if ($this->apparent) {
            return $this;
        }
        ...
        // If topographic location set, but no weather, return topo apparent
        if ($this->topo && !($pressure || $temp || $humidity)) {
            return $this->ICRStoTopo();
        }

        // If topographic location set, and weather, return topo observed
        if ($this->topo && ($pressure || $temp || $humidity)) {
            return $this->ICRStoObserved();
        }

        throw new Exception(
          'An error has occurred finding apparent coordinates');
    }
    ...
    /**
     * Finds the Earth's true obliquity for this instances observational epoch
     * if the instance is apparent, or returns the mean obliquity if it is
     * astrometric
     *
     * @return Angle
     */
    private function obli()
    {
    ...
    /**
     * Performs a [IRCS -> geocentric apparent] transformation for the
     * parameters of this instance
     *
     * @return static
     */
    private function ICRStoApparentGeo()
    {
     ...
     * @return static|Horiz
     */
    private function ICRStoTopo($type = 'e')
    {
    ...
    /**
     * Performs a [IRCS -> observed] coordinate transformation for the
     * parameters of this instance
     *
     * @param  string $type Coordintate type, 'e' for equat 'h' for horiz
     *
     * @return static|Horiz
     */
    private function ICRStoObserved(
      $type = 'e',
      Pressure $pressure = null,
      Temperature $temp = null,
      $humidity = null
        ...
        $hm    = $this->obsrv ? $this->obsrv->height->m : 0;
        ...
        //
        // CIRS -> topocentric
        IAU::Atio13($ri, $di, $utc1, 0, $dut1, $elong, $phi, $hm, $xp, $yp,
          $phpa,
        ...
        if ($type == 'e') {
            ...
            $horiz = new Horiz(
              Angle::rad(deg2rad(90) - $zob), Angle::rad($aob), $this->dist);
